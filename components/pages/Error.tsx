  ...
  return (
    ...
    >
      ...
      <Card>
        <span
          id="404-icon"
          className="rounded-2xl bg-orange-100 p-4 text-4xl text-orange-700"
        >
          {iconMap({ icon: "alert", className: "h-5 w-5" })}
        </span>
        <h1 className="text-polytime-teal-dark eyebrow">
          {global ? "Global error" : "Internal error"}
        </h1>
        <h2 className="max-w-xl text-3xl font-bold tracking-tight">
          Something went wrong
        </h2>
        <p className="text-polytime-muted max-w-xl text-sm leading-6">
          The app encountered a critical error and cannot continue. Your saved
          entries are safe, try again of return to the overview page while the
          service recovers.
        </p>
        <div
        ...
        >
          ...
          {requestId && (
            <p>
              Request with ID <code>{requestId}</code> failed. Contact the
              developer with this code for troubleshooting.
            </p>
          )}
          {global && (
            <div className="flex flex-col gap-2">
              <p>
                The application encountered a global error. Contact the
                developper with the following information for troubleshooting.
              </p>
        ...
        <div
          ...
          className="flex justify-center gap-4 text-xs font-semibold"
        >
          {!global && (
          ...
          <Button link primary={global} href={"/"}>
            Return to overview
          </Button>
        </div>
      </Card>
